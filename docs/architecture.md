...
├── research/                 # 12 документов решений + полный разбор Coldcard
...
├── .claude-plugin/
│   └── plugin.json           # манифест (имя, версия, хуки)
...
печатается в конце пейлоада: `Версия плагина: 0.7.0`. Это единственное
...
Кодируется через `jq` (или python3-фолбэк), `ensure_ascii=False` — кириллица
остаётся читаемой.
...
- **Копий нет.** Все хуки всех пяти рантаймов ссылаются на файлы проекта
  напрямую (`$PLUGIN_ROOT/...`). Правка файла в проекте = правка
  установленного плагина для всех агентов сразу; следующая сессия любого
  агента получит новый текст.
- **Хук — единственный рантайм-компонент.** У него нет состояния, кроме
  журнала сессий Hermes в `$TMPDIR`; он ничего не пишет в проект.
- **Зависимости минимальны.** `bash` + `jq` **или** `python3` (для
  JSON-кодирования). Установщику нужен `python3`.
